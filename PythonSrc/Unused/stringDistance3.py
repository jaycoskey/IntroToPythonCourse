...
class DynTable:
    def __init__(self, m, n):
        # print('Dimensions: m,n={0:d},{1:d}'.format(m, n))
        self.cells = [[DynCell(0, 0, 0, Transform.none)] * (m+1) for k in range(n+1) ]
    def __getitem__(self, key):
        (i, j) = key
        # print('Key: i,j={0:d},{1:d}'.format(i, j))
...
def xform_str(xform):
    if xform == Transform.none:
        return '_'
    elif xform == Transform.deletion:
        return 'D'
    elif xform == Transform.insertion:
        return 'I'
    elif xform == Transform.substitution:
        return 'S'
    elif xform == Transform.transposition:
        return 'T'
    else:
...
def damerauLevenshteinDistance(s1, s2, verbose=False):
    m = len(s1)
    n = len(s2)
    dynTable = damerauLevenshteinDynTable(s1, s2, verbose)
    return dynTable[(m, n)].dist


def damerauLevenshteinDynTable(s1, s2, verbose=False):
    m = len(s1)
    n = len(s2)
    ...
    for i in range(1, m + 1):
        ...
        for j in range(1, n + 1):
            ...
            if s1[i - 1] == s2[j - 1]:
                ...
                jPrevMatch = j
                dynTable[i, j].dist = newCost
                dynTable[i, j].iPrev = i - 1
                dynTable[i, j].jPrev = j - 1
                dynTable[i, j].xform = Transform.none
            else:
                ...
                if iPrev > 0 and jPrev > 0:
                    ...
                    prevDist = dist[iPrev - 1, jPrev - 1]
                    transCost = prevDist + tDelCost + tInsCost + tTransCost
                else:
                    transCost = sys.maxsize
                newCost = min(delCost, insCost, subCost, transCost)
                dynTable[(i, j)].dist = newCost
                if newCost == delCost:
                    dynTable[(i, j)].xform = Transform.deletion
                    dynTable[(i, j)].iPrev = i - 1
                    dynTable[(i, j)].jPrev = j
                elif newCost == insCost:
                    dynTable[(i, j)].xform = Transform.insertion
                    dynTable[(i, j)].iPrev = i
                    dynTable[(i, j)].jPrev = j - 1
                elif newCost == subCost:
                    dynTable[(i, j)].xform = Transform.substitution
                    dynTable[(i, j)].iPrev = i - 1
                    dynTable[(i, j)].jPrev = j - 1
                elif newCost == transCost:
                    dynTable[(i, j)].xform = Transform.transposition
                    dynTable[(i, j)].iPrev = iPrev - 1
                    dynTable[(i, j)].jPrev = jPrev - 1
                else:
                    raise Exception('unrecognized Transform type')
            dist[i, j] = newCost
    ...
    if verbose == True:
        ...
        for i in range(m + 1):
            ...
            print('    ')
            for j in range(n + 1):
                print('{0:s}  '.format(xform_str(dynTable[(i, j)].xform)), end='')
                # print('{0:d}  '.format(dynTable[(i, j)].dist), end='')
        print('')
    return dynTable
...
if __name__ == '__main__':
    test()
    d = damerauLevenshteinDistance('abcdef', 'bacefg', True)
