...
# Bugs:
#   (1) typo in is_winner
#   (2) Casing of string argument to set_board() in main().
#   (3) is_winner() only checks for 7 of the 8 possible wins.
# Homework:
...
class Game:
    ...
    def is_winner(self, player):
        ...
        if ((player == self.board[0][0] == self.board[1][1] == self.board[2][2])
            or (player == self.board[2][2] == self.board[1][1] == self.board[0][0])):
            return True
        return False
