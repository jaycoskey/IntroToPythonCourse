...
class TicTacToe:
    ...
    def __init__(self):
        self.board = [[playerToken[Player.none] for col in range(3)] for row in range(3)]
    ...
    def set_token(self, player, posn):
        row, col = divmod(posn - 1, TicTacToe.BOARD_SIZE)
        self.board[col][row] = player

    def print(self):
        hseparator = '---' + ('+---' * (TicTacToe.BOARD_SIZE - 1)) + '\n'
        hsep = ''
        for col in range(TicTacToe.BOARD_SIZE):
            print(hsep, end='')
            hsep = hseparator
            ...
            for row in range(TicTacToe.BOARD_SIZE):
                print(vsep, end='')
                vsep = vseparator
                print(' {0:s} '.format(self.board[col][row]), end='')
        print('')
